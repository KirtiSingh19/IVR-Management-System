 ...
 * Where the API lives.
 *
 * Empty, meaning same-origin: the dev server proxies /api to the Python process,
 * so the browser only ever talks to one origin. That is what makes the HttpOnly
 * session cookie work — it rides along automatically on same-origin requests,
 * and no script can read it.
 *
 * Set `window.__IVR_API_BASE__` before the app loads to point a deployment
 * elsewhere, but note that a cross-origin API would need the cookie relaxed to
 * SameSite=None; Secure, and therefore HTTPS.
 */
export const API_BASE = (window.__IVR_API_BASE__ ?? '').replace(/\/$/, '');

/**
 * Told when the API rejects us as unauthenticated.
 *
 * Registered by services/auth.js rather than imported from it, so this module
 * keeps knowing nothing about sessions and the two do not depend on each other
 * in a circle.
 */
let markSignedOut = () => {};

export function onUnauthorised(handler) {
  markSignedOut = handler;
}
...
async function request(method, path, body) {
  ...
  if (!response.ok) {
    // An expired or revoked session shows up here as a 401 on whatever the page
    // happened to be loading. Telling the auth module flips the guard, which
    // renders <Navigate> to /login — no document reload, so the SIP module and
    // any call in progress are left alone.
